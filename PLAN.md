...
- [x] Resend transactional email: Supabase Edge Function `send-email` with templates for invitations, pass confirmations, gallery-live alerts, check-in confirmations, and expiry warnings. Client-side wrapper in `lib/email.ts`. API key stored as Supabase secret — never exposed to the client.
